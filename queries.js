...
db.restaurants.find({ cuisine: {$ne: "American "}, "grades.grade":{$eq: 'A'}, borough: {$ne: "Brooklyn"}})
...
db.restaurants.find({ cuisine: {$all: ["Seafood"], $not: { $in: ["American ", "Chinese"]}}, name: {$not: /^Wil/}}, { restaurant_id: 1, name: 1, borough: 1, cuisine: 1})
...
db.restaurants.find({"address.street":{$ne: ""}})
db.restaurants.find({"address.coord":{$type: "double"}})
db.restaurants.aggregate([{$match: { grades: { $elemMatch: {score: { $mod: [7, 0] }}}}}]);
